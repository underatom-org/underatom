...
import { Description, Label, type FieldProps } from "@/registry/ui/default/core/field";
    ...
    <SliderRoot ref={ref} {...props}>
      {(label || !!valueLabel) && (
        <div className={cn("flex items-center justify-between gap-2", !label && "justify-end")}>
          ...
          {!!valueLabel && (
            <SliderValueLabel>
              {({ state }) => (typeof valueLabel === "function" ? valueLabel(state.values) : undefined)}
            </SliderValueLabel>
          )}
        </div>
      )}
...
const SliderTrack = ({ size, ...props }: SliderTrackProps) => {
  ...
  return (
    <AriaSliderTrack {...props} className={composeRenderProps(props.className, (className) => track({ className }))} />
  );
};
...
const SliderThumb = ({ size, ...props }: SliderThumbProps) => {
  ...
  return (
    <AriaSliderThumb {...props} className={composeRenderProps(props.className, (className) => thumb({ className }))} />
  );
};
...
export { Slider, SliderRoot, SliderControls, SliderTrack, SliderFiller, SliderThumb, SliderValueLabel };
